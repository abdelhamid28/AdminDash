...
export class TermsComponent implements OnInit {
  ...
  getList() {
    this.api.get_private('v1/terms/getAll').then((data: any) => {
      this.dummy = [];
      if (data && data.status && data.status == 200) {
        this.list = data.data;
      }
  ...
  saveData() {
    if (this.cover === '' || this.cover === null || this.name === '' || this.name === null || this.studies_id == '') {
    ...
    this.util.show();
    this.api.post_private('v1/terms/save', param).then((data: any) => {
      console.log(data);
      this.util.hide();
      if (data && data.status && data.status == 200) {
        this.getList();
  ...
  statusUpdate(item: any) {
    ...
    Swal.fire({
      ...
      if (data && data.value) {
        ...
        this.util.show();
        this.api.post_private('v1/terms/update', item).then((data: any) => {
          console.log(data);
          this.util.hide();
  ...
  updateData() {
    ...
    this.util.show();
    this.api.post_private('v1/terms/update', param).then((data: any) => {
      console.log(data);
      this.util.hide();
      if (data && data.status && data.status == 200) {
        this.getList();
